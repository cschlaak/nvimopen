...
const NEOVIDE_NAME: &str      = "neovide";
...
fn main() {
	...
	if !is_server_running() {
		// spawn this process (blocking) to wait until the server is running
		init_command(NEOVIDE_NAME).arg("--").arg("--listen").arg(SERVER).spawn().expect(COMMAND_ERROR_MSG);

		// allow some additional time to pass for neovim's initialization
